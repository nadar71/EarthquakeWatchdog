...
class ProductionOperationsDocsTest(unittest.TestCase):
    ...
    def test_background_location_evidence_distinguishes_dependency_service(self) -> None:
        documents = (
            self.read("docs/release/play-compliance-checklist.md"),
            self.read("docs/release/production-readiness-checklist.md"),
        )

        for document in documents:
            self.assertIn(
                "no app-authored foreground service or background-location worker",
                document,
            )
            self.assertIn("SystemForegroundService", document)
            self.assertNotIn("no background permission/service exists", document)
            self.assertNotIn(
