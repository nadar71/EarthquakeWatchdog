...
class LoadAllOrderByMostRecentUseCase @Inject constructor(
    private val repository: EQRepository,
    @ApplicationContext private val context: Context
) {
    private val TAG = "LoadAllOrderByMostRecentUseCase"

    suspend operator fun invoke(minMag: Double): DbResponse<List<EQEntity>> {
        return try {
            val result = repository.loadAll_orderby_most_recent(minMag)
