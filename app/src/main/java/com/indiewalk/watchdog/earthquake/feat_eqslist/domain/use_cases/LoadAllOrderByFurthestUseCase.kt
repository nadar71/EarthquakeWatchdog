...
class LoadAllOrderByFurthestUseCase @Inject constructor(
    private val repository: EQRepository,
    @ApplicationContext private val context: Context
) {
    private val TAG = "LoadAllOrderByFurthestUseCase"

    suspend operator fun invoke(minMag: Double): DbResponse<List<EQEntity>> {
        return try {
            val result = repository.loadAll_orderby_furthest(minMag)
