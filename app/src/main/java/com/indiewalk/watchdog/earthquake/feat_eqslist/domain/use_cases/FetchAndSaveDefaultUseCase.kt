...
class FetchAndSaveDefaultUseCase @Inject constructor(
    private val repository: EQRepository,
    @ApplicationContext private val context: Context
) {
    private val TAG = "FetchAndSaveDefaultUseCase"

    suspend operator fun invoke(): ApiResponse<EQFeaturesCollectionDTO> {
        return try {
            val response = repository.fetchAndSaveDefault()
            ApiResponse.Success(response)
            // TODO: manage http 400 & c.
        } catch (e: Exception) {
