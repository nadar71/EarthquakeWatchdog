...
class MainActivity : AppCompatActivity() {
                ...
                if (item == 0) {       // Show on Map
                    ...
                    if (nDays <= DAYS_LIMIT) {
                        ...
                        showEqOnMap.putExtra(
                            "equake_lat",
                            Double.toString(earthquake.latitude)
                        )
                        showEqOnMap.putExtra(
                            "equake_lng",
                            Double.toString(earthquake.longitude)
                        )
    ...
    private fun checkPreferences() {
        ...
        lat_s = sharedPreferences.getString(
            getString(R.string.device_lat), Double.toString(
                DEFAULT_LAT
            )
        )
        lng_s = sharedPreferences.getString(
            getString(R.string.device_lng), Double.toString(
                DEFAULT_LNG
            )
        )
