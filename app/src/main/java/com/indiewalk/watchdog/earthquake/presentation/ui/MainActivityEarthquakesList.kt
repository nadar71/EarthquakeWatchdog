...
class MainActivityEarthquakesList : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnFilterDetails.summaryBox.visibility = View.GONE
    ...
    // Check if device location is stored by previous accessing in-map section, and in case retrieve coordinates
    ...
    private fun initRecycleView() {
        ...
        // at the moment it has only tens of entry.
        ...
        */
        ...
        earthquakeListView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            ...
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                if (dy > 0 || dy < 0 && binding.infoFilterFab.isShown)
                    binding.infoFilterFab.hide()
            }

            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                if (newState == RecyclerView.SCROLL_STATE_IDLE)
                    binding.infoFilterFab.show()
                super.onScrollStateChanged(recyclerView, newState)
            }
        })

        binding.infoFilterFab.setOnClickListener {
            if (binding.btnFilterDetails.summaryBox.isShown)
                binding.btnFilterDetails.summaryBox.visibility = View.GONE
            else
                binding.btnFilterDetails.summaryBox.visibility = View.VISIBLE
        }
    }
            ...
            .setItems(items) { dialog, item ->
                if (item == 0) {       // Show on Map
                    ...
                    if (nDays <= DAYS_LIMIT) {
                        ...
                        Toast.makeText(
                            this@MainActivityEarthquakesList,
                            getString(R.string.use_of_map_forbidden), Toast.LENGTH_LONG
                        ).show()
                    }
    ...
    private fun setFilterSummary() {
        ...
        // order by
        if (orderBy == getString(R.string.settings_order_by_desc_magnitude_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_desc_magnitude_label)

        if (orderBy == getString(R.string.settings_order_by_asc_magnitude_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_asc_magnitude_label)

        if (orderBy == getString(R.string.settings_order_by_most_recent_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_most_recent_label)

        if (orderBy == getString(R.string.settings_order_by_oldest_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_oldest_label)

        if (orderBy == getString(R.string.settings_order_by_nearest_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_nearest_label)

        if (orderBy == getString(R.string.settings_order_by_furthest_value))
            binding.btnFilterDetails.orderValueTv.text =
                getString(R.string.settings_order_by_furthest_label)

        // min magnitude
        binding.btnFilterDetails.minMagnValueTv.text = minMagnitude

        // last update time
        binding.btnFilterDetails.lastUpValueTv.text = lastUpdate

        // time range
        if (dateFilter == getString(R.string.settings_date_period_today_value))
            binding.btnFilterDetails.eqPeriodValueTv.text =
                getString(R.string.settings_date_period_today_label)
        else if (dateFilter == getString(R.string.settings_date_period_24h_value))
            binding.btnFilterDetails.eqPeriodValueTv.text =
                getString(R.string.settings_date_period_24h_label)
        else if (dateFilter == getString(R.string.settings_date_period_48h_value))
            binding.btnFilterDetails.eqPeriodValueTv.text =
                getString(R.string.settings_date_period_48h_label)
        else if (dateFilter == getString(R.string.settings_date_period_week_value))
            binding.btnFilterDetails.eqPeriodValueTv.text =
                getString(R.string.settings_date_period_week_label)
        else if (dateFilter == getString(R.string.settings_date_period_2_week_value))
            binding.btnFilterDetails.eqPeriodValueTv.text =
                getString(R.string.settings_date_period_2_week_label)/*
        else if ((dateFilter.equals(getString(R.string.settings_date_period_3_days_value))))
            dateFilterLabel = getString(R.string.settings_date_period_3_days_label);
        ...
        */
        ...
        //location address
        binding.btnFilterDetails.locationValueTv.text = location_address
    }
    ...
    private fun showNoInternetConnection() {
        ...
        binding.btnFilterDetails.filterSummary.visibility = View.INVISIBLE
        earthquakeListView.visibility = View.GONE
        binding.emptyListText.visibility = View.VISIBLE
        binding.emptyListText.setText(R.string.no_internet_connection)
    }
    ...
    private fun showLoading() {
        ...
        earthquakeListView.visibility = View.GONE
        binding.emptyListText.visibility = View.VISIBLE
        binding.emptyListText.setText(R.string.searching)
    }

    // Show hearthquake list after loading/retrieving data completed
    private fun showEarthquakeListView() {
        binding.loadingInProgress.visibility = View.INVISIBLE
        binding.btnFilterDetails.summaryBox.visibility = View.VISIBLE
    ...
    override fun onLoadFinished(
        ...
        if (setEartquakesList(earthquakesReturnedByLoader)) {
            ...
            // update preferences
            lastUpdate = GenericUtils.setLastUpdateField(this)
    ...
    protected fun setEartquakesList(earthquakes: List<Earthquake>?): Boolean {
        if (!earthquakes.isNullOrEmpty()) {
            ...
            Toast.makeText(
                this,
                "The earthquake list is empty. Check the request. ",
                Toast.LENGTH_LONG
            ).show()
            return false
        }
        ...
        when (item.itemId) {
            ...
            R.id.set_myposition_action -> {
                ...
                if (nDays <= DAYS_LIMIT) {
                    ...
                    Toast.makeText(
                        this,
                        getString(R.string.use_of_map_forbidden),
                        Toast.LENGTH_LONG
                    ).show()
                }
                return true
    ...
    private fun showDialog() {
        val builder = AlertDialog.Builder(this@MainActivityEarthquakesList)
        ...
        */
        ...
        builder.setPositiveButton("Ok") { dialog, which ->
            ...
            if (!spinner_min_magn_choice.equals(
                    resources.getString(R.string.spinner_defaultchoice_value),
                    ignoreCase = true
                )
            ) {
                editor.putString(
            ...
            dialog.dismiss()
            GenericUtils.restartActivity(this@MainActivityEarthquakesList)
        }
    ...
    private fun convertFromLabelsToKeyvalues(
        selectedItem: String,
    ...
    companion object {
        ...
        const val ORDER_BY_ASC_MAGNITUDE = "magnitude_asc_ordering"
        const val ORDER_BY_MOST_RECENT = "most_recent"
        const val ORDER_BY_OLDEST = "oldest"
        const val ORDER_BY_NEAREST = "nearest"
        const val ORDER_BY_FURTHEST = "furthest"
