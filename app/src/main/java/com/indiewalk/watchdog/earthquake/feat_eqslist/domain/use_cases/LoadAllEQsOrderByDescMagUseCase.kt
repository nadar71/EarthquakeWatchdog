...
class LoadAllEQsOrderByDescMagUseCase @Inject constructor(
    private val repository: EQRepository,
    @ApplicationContext private val context: Context
) {
    private val TAG = "LoadAllEQsOrderByDescMagUseCase"

    suspend operator fun invoke(minMag: Double): DbResponse<List<EQEntity>> {
        return try {
            val result = repository.loadAllEQs_orderby_desc_mag(minMag)
