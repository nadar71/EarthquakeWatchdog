...
class LoadAllEQsOrderByAscMagUseCase @Inject constructor(
    private val repository: EQRepository,
    @ApplicationContext private val context: Context
) {
    private val TAG = "LoadAllEQsOrderByAscMagUseCase"

    suspend operator fun invoke(minMag: Double): DbResponse<List<EQEntity>> {
        return try {
            val result = repository.loadAllEQs_orderby_asc_mag(minMag)
