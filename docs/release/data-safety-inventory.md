...
| Google Mobile Ads SDK | IP-derived approximate location, app interactions, diagnostics, device/account advertising identifiers, and network metadata as described by the SDK. | Advertising, analytics, fraud prevention/security, personalization according to consent/configuration. | Approximate location, app activity, diagnostics, device/other IDs are collection/share candidates. Exact answers remain `PENDING`. | `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_ads/domain/AdsConsentCoordinator.kt`; `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_ads/data/MobileAdsSdkInitializer.kt`; `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_ads/presentation/AdMobBannerView.kt`. Protected production IDs and internal-track delivery remain owner-verified. |
| Google UMP SDK | Consent-status/request metadata, device/network identifiers needed to determine/show configured privacy messages. | Consent/privacy-choice management for ads. | App activity/device identifier handling is `PENDING` exact-version review. Never preserve consent payload values in incident evidence. | `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_eqslist/presentation/ui/MainActivity.kt`; `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_ads/data/UmpConsentClient.kt`; `app/src/main/java/com/indiewalk/watchdog/earthquake/feat_settings/presentation/ui/SettingsScreen.kt`; regional production behavior remains `PENDING`. |
